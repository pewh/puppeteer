...
class Frame {
   ...
   */
  async $(selector, isXPath=false) {
    const document = await this._document();
    const value = isXPath ?
      await document.$x(selector)[0] :
      await document.$(selector);
    return value;
  }
   ...
   * @param {string} selector
   */
  async focus(selector, isXPath=false) {
    const handle = await this.$(selector, isXPath);
    assert(handle, 'No node found for selector: ' + selector);
    await handle.focus();
    await handle.dispose();
  }
   ...
   * @param {string} selector
   */
  async hover(selector, useXpath) {
      const handle = await this.$(selector, useXpath);
      assert(handle, 'No node found for selector: ' + selector);
      await handle.hover();
      await handle.dispose();
    }
